...
!function(self){
	...
	/**
		...
		//TODO
	**/
	self.isKoreanPhoneme = function(char,autoConv){ return /^[ㄱ-ㅣ]/.test(char) || autoConv && INDEXING_ARRAY__KO_PHONEME.indexOf(char)>=0 || false; };

	/**
		@param {string}char 음소 하나를 밭습니다. 문자열인 경우 첫글자만 판단.
		@param {boolean}autoConv 주어진 phon값이 영문 알파벳인 경우 2벌식으로 자동변환하여 인식할지 여부.
		@true 문자열의 첫 글자가 자음이면서 초성에 위치할 수 있는 자음이면 true
	**/
	self.isConsonant = function(char,autoConv){ return /^[ㄱ-ㅎ]/.test(char)||autoConv&&CHAR_CODE_AT_KO_PHONEME_BEGIN<=char.charCodeAt(0)&&char.charCodeAt(0)<CHAR_CODE_AT_KO_PHONEME_BEGIN+CHAR_COUNT__KO_CONSONANT; };

	/**
		@param {string}char 음소 하나를 밭습니다.
		@param {boolean}autoConv 주어진 phon값이 영문 알파벳인 경우 2벌식으로 자동변환하여 인식할지 여부.
		@true 자음이면서 초성에 위치할 수 있는 자음이면 true
	**/
	self.isFirstSound = function(char,autoConv){ return autoConv&&Boolean(1+INDEXING_ARRAY__KO_CONSONANT.indexOf(char))||Boolean(1+INDEXING_ARRAY__KO_CONSONANT.indexOf(convertAlphabetToPhoneme(char)))||false; };
	...
	/**
		...
		@true 모음이면 true
	**/
	self.isVowel=self.isMidSound = function(char,autoConv){ var n=char.charCodeAt(0);return /^[ㅏ-ㅣ]/.test(char)||autoConv&&CHAR_COUNT__KO_CONSONANT<=INDEXING_ARRAY__KO_PHONEME.indexOf(char); };
	...
	**/
	self.isLastSound = function(char,autoConv){ return autoConv&&Boolean(1+INDEXING_ARRAY__KO_LAST_SOUND.indexOf(char))||Boolean(1+INDEXING_ARRAY__KO_LAST_SOUND.indexOf(convertAlphabetToPhoneme(char)))||false; };
	...
	**/
	self.convertAlphabetToPhoneme = function(char,defaultValue,caseInsencitive){
		...
		}else ci=caseInsencitive;
		if(INDEXING_ARRAY__KO_PHONEME.indexOf(char)>=0) return convertAlphabetToPhoneme(char);
		if(ci)
			for(var i=0;i<INDEXING_ARRAY__KO_PHONEME.length;i++)
				if(INDEXING_ARRAY__KO_PHONEME[i].toLowerCase()==char.toLowerCase())
					return convertAlphabetToPhoneme(INDEXING_ARRAY__KO_PHONEME[i]);
		return defaultValue===undefined?char:defaultValue;
	}
	...
	**/
	self.convertPhonemeToAlphabet = function(char,defaultValue){ return /ㄱ-ㅣ/i.test(char)?convertAlphabetToPhoneme(char):defaultValue; }
	...
	**/
	self.isKoreanChar=function(char){ return /^[가-힣]/.test(char); }
	...
	**/
	self.isKorean = function(str){ return /^[ㄱ-ㅣ가-힣]+$/.test(str); }

	/**
		@param {string}str 문자열을 받습니다.
		@return 입력된 문자 중 한글 문자가 처음으로 발견된 위치. 없으면 -1
	**/
	self.indexOfKorean = function(str){ var m=str.match(/[ㄱ-ㅣ가-힣]/); return m?m.index:-1; }

	/**
		@param {string}str 문자열을 받습니다.
		@return 입력된 문자 중 완성된 한글 음절이 처음으로 발견된 위치. 없으면 -1
	**/
	self.indexOfKoreanChar = function(str){ var m=str.match(/[가-힣]/); return m?m.index:-1; }

	/**
		@param {string}str 문자열을 받습니다.
		@return 입력된 문자 중 조합되지 않은 한글 음소가 처음으로 발견된 위치. 없으면 -1
	**/
	self.indexOfKoreanPhoneme = function(str){ var m=str.match(/[ㄱ-ㅣ]/); return m?m.index:-1; }
	...
	**/
	self.hasLastSound = function(str){
		...
		if(c < CHAR_CODE_AT_KO_CHAR_BEGIN) return false;
		if(c >= CHAR_CODE_AT_KO_CHAR_BEGIN+CHAR_COUNT__KO_CHAR) return false;
	...
	**/
	self.koreanify = function(str){ return convertAlphabetsToKorean(str); };;
	...
	**/
	self.qwerty = function(str){
		return self.split(str).split("").map(function(c){
			if(self.isKoreanChar(c)) return self.split(c).split("").map(c=>self.qwerty(c)).join("");
			if(self.isKoreanPhoneme(c)) return INDEXING_ARRAY__KO_PHONEME[c.charCodeAt(0)-CHAR_CODE_AT_KO_PHONEME_BEGIN];
			return c;
		}).join("");
	}
	...
	**/
	self.spread = function(str){
		return str.split("").map(function(c){
			...
			return self.qwerty(c).split("").map(c=>convertAlphabetToPhoneme(c)).join("");
		}).join("");
	}
	...
	**/
	self.compress = function(str){ return convertAlphabetsToKorean(str.split("").map(c=>self.isKoreanPhoneme(c)?convertPhonemeToAlphabet(c):c).join("")); }

	const POST_POSITION_MATCH = [
		[ ['은(는)','는(은)'] , ["는","은"]],
		[ ['(이)는'] , ["는","이는"]],
		[ ['이(가)','가(이)'] , ["가","이"]],
		[ ['(이)가'] , ["가","이가"]],
	...
	self.fixPostPositions = function(str){
		...
		for(loopLimit=100;loopLimit>=0;loopLimit--){
			var i=-1,bef,aft;
			POST_POSITION_MATCH.map(function(pair){
				pair[0].map(function(kw){ var j=str.indexOf(kw); if(j+1)if(i<0||j<i){ i=j; bef=kw; aft=null; } });
				if(aft==null&&i>=0) aft=pair[1];
			});
			if(i==-1) break;
			ret += str.substr(0,i);
			ret += aft[self.hasLastSound(ret.replace(/\(.*\)|\s/g,""))?1:0];
			str=str.substr(i+bef.length);
		}
		ret += str;
		return ret;
	}
			...
			if(wait.length!=1 && self.isVowel(c,1)){
				if(wait.length>1) --i;
					...
					if(self.isVowel(cc,1)){
						++i;
						ret += _compress([null,convertPhonemeToMidSoundIndex(cc)]);
					}
					else ret += _compress([null,convertPhonemeToMidSoundIndex(c)]);
					continue;
				}
			}else switch(wait.length){
				case 0:
					if(self.isFirstSound(c,1)){
						wait.push(convertPhonemeToFirstSoundIndex(c));
						continue;
					}
					break;
				case 1:
					if(self.isMidSound(cc,1)){ ++i;
						wait.push(convertPhonemeToMidSoundIndex(cc));
						continue;
					}
					if(self.isMidSound(c,1)){
						wait.push(convertPhonemeToMidSoundIndex(c));
						continue;
					}
					break;
				case 2:
					if(self.isLastSound(cc,1) && !self.isVowel(xc,1)){ ++i;
						wait.push(convertPhonemeToLastSoundIndex(cc));
					}else
					if(self.isLastSound(c,1) && !self.isVowel(cc[1],1)){
						wait.push(convertPhonemeToLastSoundIndex(c));
					}
					if(wait.length>2) ++i;
					break;
			}
	...
	/** 알파벳 문자를 한글 음소로 변환 */ function convertAlphabetToPhoneme(c){ return String.fromCharCode(CHAR_CODE_AT_KO_PHONEME_BEGIN+INDEXING_ARRAY__KO_PHONEME.indexOf(c)); }
	/** 한글 음소를 알파벳 문자로 변환 */ function convertPhonemeToAlphabet(c,d){ return INDEXING_ARRAY__KO_PHONEME[c.charCodeAt(0)-CHAR_CODE_AT_KO_PHONEME_BEGIN]; }
	/** 주어진 음소가 초성으로서 몇 번째 순번인지, 초성으로 볼 수 없다면 -1를 반환 */ function convertPhonemeToFirstSoundIndex(phon){ return INDEXING_ARRAY__KO_CONSONANT.indexOf(phon); }
	/** 주어진 음소가 중성으로서 몇 번째 순번인지, 중성으로 볼 수 없다면 -1를 반환 */ function convertPhonemeToMidSoundIndex(phon){ var i=INDEXING_ARRAY__KO_PHONEME.indexOf(phon)-CHAR_COUNT__KO_CONSONANT; return i<0?-1:i; }
	/** 주어진 음소가 종성으로서 몇 번째 순번인지, 종성으로 볼 수 없다면 -1를 반환 */ function convertPhonemeToLastSoundIndex(phon){ return INDEXING_ARRAY__KO_LAST_SOUND.indexOf(phon); }
